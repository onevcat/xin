...
/// Fastmail appears to validate `header:*:asMessageIds` fairly strictly when creating drafts.
/// Some real-world Message-IDs (e.g. generated by Gmail) can include tokens that Fastmail rejects
/// via `invalidProperties (properties: header:In-Reply-To)`.
///
/// This fallback builds raw RFC 5322 header text via `:asText` so we can still send a reply.
fn build_reply_headers_raw_text(
    original: &jmap_client::email::Email,
) -> Result<Vec<(jmap_client::email::Header, jmap_client::email::HeaderValue)>, XinErrorOut> {
    use jmap_client::email::{Header, HeaderValue};
    ...
    fn wrap_angle_brackets(token: &str) -> String {
        let t = token.trim();
        if t.is_empty() {
            "".to_string()
    ...
    let in_reply_to_text = wrap_angle_brackets(&orig_msg_id);
    let references_text = refs_out
        .iter()
        .map(|t| wrap_angle_brackets(t))
        .filter(|s| !s.is_empty())
        .collect::<Vec<_>>()
        .join(" ");
    ...
    Ok(vec![
        (
            Header::as_text("In-Reply-To", false),
            HeaderValue::AsText(in_reply_to_text),
        ),
        (
            Header::as_text("References", false),
            HeaderValue::AsText(references_text),
        ),
    ])
}
...
pub async fn reply(account: Option<String>, args: &ReplyArgs) -> Envelope<Value> {
    ...
    fn is_fastmail_invalid_reply_headers(e: &XinErrorOut) -> bool {
        e.kind == "jmapRequestError"
            && e.message.contains("invalidProperties")
            && (e.message.contains("header:In-Reply-To") || e.message.contains("header:References"))
    }
        ...
        .await
    {
        Ok(e) => e,
        Err(e) => {
            if is_fastmail_invalid_reply_headers(&e) {
                // Fallback: use raw header text if Fastmail rejects parsed `:asMessageIds`.
                let headers_raw = match build_reply_headers_raw_text(&original) {
                    Ok(h) => h,
                    Err(e2) => return Envelope::err("reply", account, e2),
                };
                ...
                return Envelope::err("reply", account, e);
            }
        }
    };
